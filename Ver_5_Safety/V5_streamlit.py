...
def display_target_goals():
    ...
    with st.container():
        ...
        # 상세 실행 계획 표시
        if 'detail_plan' in target_data and target_data['detail_plan'] and pd.notna(target_data['detail_plan']):
            with st.expander("📋 상세 실행 계획 (클릭하여 펼치기)", expanded=False):
                st.markdown(f"""
                    <div class="detail-plan-container">
                        <div class="detail-plan-title">🔍 구체적 실행 계획</div>
                        <div class="detail-plan-content">{target_data['detail_plan']}</div>
                    </div>
                """, unsafe_allow_html=True)
        else:
            st.info("상세 실행 계획이 설정되지 않았습니다.")
