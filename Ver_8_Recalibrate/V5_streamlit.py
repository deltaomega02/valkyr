    ...
    with sqlite3.connect(db_path) as conn:
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS decision_targets (
                ...
                detail_reason TEXT,                 -- 목표가 설정 근거 (추가 진입/매도 가격, 조건부 행동 등)
...
def get_next_execution_time():
    ...
    # 뉴스 포함 분석 시간 (주요 시장 시간대)
    news_times = ["09:05", "17:05", "22:05"]
    
    # 매시간 정기 분석 시간 (뉴스 제외)
    regular_times = []
    for hour in range(24):
        hour_str = f"{hour:02d}:05"
        if hour_str not in news_times:  # 뉴스 시간과 겹치지 않게
            regular_times.append(hour_str)
    ...
    # 다음 실행이 뉴스 포함 시간인지 확인
    is_news_time = next_time.strftime("%H:%M") in news_times
    next_time_type = "뉴스 포함" if is_news_time else "일반"
    
    return (
        next_time.strftime("%Y-%m-%d %H:%M"),
        f"{hours:02d}:{minutes:02d}",
        sorted(all_times),  # 모든 분석 시간을 정렬하여 반환
        next_time_type  # 다음 실행 유형 반환
    )
...
def display_collapsible_schedule_summary():
    ...
    next_time, remaining, _, next_time_type = get_next_execution_time()
    
    # 타입에 따른 색상 설정
    type_color = "#4CAF50" if next_time_type == "뉴스 포함" else "#1976d2"
    
    # 기본 타이머 표시 - 간소화된 버전, 툴팁 없이
    st.markdown(f"""
        <div class="countdown-container">
            ...
            <div class="next-analysis">
                ...
                <div class="next-analysis-type" style="color: {type_color};">{next_time_type} 분석</div>
            </div>
        </div>
    """, unsafe_allow_html=True)

# 시간대별 시장 설명 반환
def get_market_description(time):
    if time == "09:05":
        return "아시아/한국 시장 활동 시간"
    elif time == "17:05":
        return "유럽 시장 활발 / 미국 시장 개장 전"
    elif time == "22:05":
        return "미국 시장 가장 활발한 시간"
    else:
        return ""
            ...
            with st.expander("📋 목표가 설정 근거 (클릭하여 펼치기)", expanded=False):
                st.markdown(f"""
                    <div class="detail-plan-container">
                        <div class="detail-plan-title">🔍 목표가 설정 근거</div>
                        <div class="detail-plan-content">{target_data['detail_reason']}</div>
                    </div>
                """, unsafe_allow_html=True)
        else:
            st.info("목표가 설정 근거가 설정되지 않았습니다.")
